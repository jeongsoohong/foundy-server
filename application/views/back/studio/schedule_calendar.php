    ...
    <tr>
      <?php for ($week = 0; $week < 7; $week++) { ?>
        <td>
          ...
          <?
          $where = array('studio_id' => $studio_id, 'schedule_date' => "{$y}-{$m}-{$d}", 'activate' => 1);
          $schedules = $this->db->order_by('start_time', 'asc')->get_where('studio_schedule_info', $where)->result();
          ?>
  ...
  $(function() {
    ...
    $('.case-time').click(function(){
      ...
      if (parseInt(_m) !== month) {
        ...
        get_page('schedule_list', '<?= base_url(); ?>studio/schedule/list?date=' + date , true);
        target.show();
